...
* We added new filtering capabilities to our offline data retrieval API (`mx.data.getSlice`). Now you can group constraints with `and` and `or` operators.
...
* When a radio button used an attribute for which the user did not have read access, an error was displayed. We fixed that for you. (Ticket 56467)
